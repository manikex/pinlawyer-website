...
import { MapPin, Menu, X, ChevronDown, Phone } from 'lucide-react';
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          {/* Sidebar – solid dark slate, with scrolling */}
          <motion.aside
            initial={{ x: '100%' }}
            animate={{ x: 0 }}
            exit={{ x: '100%' }}
    ...
    <header className="sticky top-0 z-50 bg-slate-900/95 backdrop-blur border-b border-slate-700">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-6 text-sm font-medium text-slate-300">
          ...
          <Link
            href="/contact/urgent"
            className="ml-4 px-4 py-2 bg-amber-500 text-slate-900 rounded-lg font-semibold hover:bg-amber-400 transition flex items-center gap-1"
          >
